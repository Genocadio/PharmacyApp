...
import 'package:nexxstore/services/invoice_service.dart';
import 'package:nexxstore/services/dto/stock_out_dto.dart';
import 'package:nexxstore/ui/widgets/compact_date_range_picker.dart';
...
import 'package:nexxstore/ui/screens/stock_request_screen.dart';
import 'package:nexxstore/ui/screens/catalog_screen.dart';
import 'package:nexxstore/ui/screens/profile_screen.dart';
import 'package:nexxstore/ui/screens/settings_screen.dart';
import 'package:nexxstore/ui/screens/user_management_screen.dart';
...
class _StockInOutScreenState extends State<StockInOutScreen> {
  ...
  /// Show invoice preview dialog with pre-generated PDF
  /// Pre-generating the PDF prevents regeneration when background sync triggers rebuilds
  Future<void> _showInvoicePreview(StockOutDTO stockOut) async {
    if (!mounted) return;
    ...
    try {
      pdfBytes = await _buildInvoicePdf(stockOut);
    } catch (e) {
    ...
    // Show PDF preview dialog with cached bytes
    // Using cached bytes prevents regeneration when services trigger notifyListeners
    if (pdfBytes != null) {
      final cachedPdfBytes = pdfBytes; // Capture as non-nullable for closure
      showDialog(
        context: context,
        builder: (dialogContext) {
          return Dialog(
            ...
            child: SizedBox(
              ...
              child: PdfPreview(
                build: (format) async => cachedPdfBytes,
            ...
            Row(
              ...
              children: [
                IconButton(
                  ...
                  onPressed: () => _showInvoicePreview(stockOut),
                ),
                IconButton(
